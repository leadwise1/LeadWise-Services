  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="bg-[#232136] p-6 text-white">
          ...
          <div className="flex justify-between pt-4 border-t mt-4">
            ...
            {step < 3 ? (
              <button type="button" onClick={() => {
                  ...
                  setStep(step + 1);
                }} className="bg-[#232136] text-white px-6 py-2 rounded-lg font-bold">Next Step</button>
            ) : (
              <button type="submit" disabled={loading} className="bg-[#fac0ab] text-[#232136] px-8 py-2 rounded-lg font-bold hover:bg-yellow-400">
  ...
  return (
    <a
      ...
      className={`flex items-start gap-3 p-3 rounded-lg transition-colors duration-200 group relative ${
        isEnrolled ? "bg-[#232136]/10 hover:bg-[#232136]/20 cursor-pointer" : "bg-gray-100 hover:bg-gray-200 cursor-not-allowed"
      }`}
    >
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <p className="text-sm font-semibold text-[#232136] group-hover:text-opacity-90">
            {resource.title}
          </p>
          {!isEnrolled && (
      ...
      <span className="text-[#232136] group-hover:text-opacity-90 mt-1">
    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200">
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between p-6 bg-white hover:bg-gray-50 transition-colors duration-200">
        <div className="text-left flex-1">
          <h3 className="text-lg font-bold text-[#232136] mb-1">{module.title}</h3>
          <p className="text-sm text-gray-600">Duration: {module.duration}</p>
        </div>
        {expanded ? <ChevronUp className="w-5 h-5 text-[#232136]" /> : <ChevronDown className="w-5 h-5 text-[#232136]" />}
      </button>
          ...
          <div className="space-y-4 p-6">
            {module.lessons.map((lesson) => (
              <div key={lesson.id} className="bg-white rounded-lg p-4 border border-gray-100">
                <h4 className="text-base font-semibold text-[#232136] mb-4">{lesson.title}</h4>
...
function CourseCard({ course, isEnrolled, onTriggerIntake }: { course: Course, isEnrolled: boolean, onTriggerIntake: () => void }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-shadow duration-300">
      <div className="bg-gradient-to-r from-[#232136] to-[#232136]/80 text-white p-8">
      ...
      <div className="p-8">
        <div className="mb-8">
          <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between bg-[#232136]/10 hover:bg-[#232136]/20 p-4 rounded-lg transition-colors duration-200 mb-6">
            <span className="font-semibold text-[#232136]">{expanded ? "Hide" : "Show"} Course Modules</span>
            {expanded ? <ChevronUp className="w-5 h-5 text-[#232136]" /> : <ChevronDown className="w-5 h-5 text-[#232136]" />}
          </button>
        ...
        <div className="border-t border-gray-200 pt-6">
          {!isEnrolled ? (
            <button onClick={onTriggerIntake} className="w-full bg-[#fac0ab] text-[#232136] hover:bg-yellow-400 font-bold py-3 px-6 rounded-lg transition-colors duration-200">
...
export default function Academy() {
  ...
  const handleNavButtonClick = () => {
    if (isEnrolled) {
      // Scroll to course content if already enrolled
      const coursesSection = document.getElementById("featured-paths");
      if (coursesSection) {
        coursesSection.scrollIntoView({ behavior: "smooth" });
      }
    } else {
      // Open intake modal if not enrolled
      setIntakeOpen(true);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="bg-[#232136] sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link href="/" className="text-gray-100 hover:text-white transition">Home</Link>
            <Link href="/#templates" className="text-gray-100 hover:text-white transition">Templates</Link>
            <Link href="/courses" className="text-white font-semibold">Courses</Link>
            {/* Added Link to Blog Here */}
            <a href="https://blog.letsleadwise.org" target="_blank" rel="noopener noreferrer" className="text-gray-100 hover:text-white transition">Blog</a>
            
            <button 
              onClick={handleNavButtonClick}
              className="bg-[#fac0ab] hover:bg-yellow-200 text-[#232136] font-semibold py-2 px-6 rounded-lg transition-colors duration-200"
            >
      ...
      <section className="bg-gradient-to-br from-[#232136]/5 to-white py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl font-bold text-[#232136] mb-6 leading-tight">Level Up Your Skills</h1>
          ...
          <div className="inline-block bg-[#fac0ab] text-[#232136] px-4 py-2 rounded-full text-sm font-semibold">✓ 100% Free • Self-Paced • Industry-Relevant</div>
        </div>
      </section>
      ...
      <section id="featured-paths" className="py-20 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-[#232136] mb-4">Featured Learning Paths</h2>
